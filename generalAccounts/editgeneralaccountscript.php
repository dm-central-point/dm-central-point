...
if(!empty($fm_date) & !empty($fm_month)& !empty($fm_year)  & !empty($fm_descr) & !empty($fm_type) ){
    ...
    $insert = mysqli_query($connect, "INSERT INTO `overall_accounts_changebackup`( `month_id`, `year_id`, `transactionDate`, `transactionMonth`, `transactionYear`, `itemDescription`, `transactionType`, `incomeAmount`, `expenditureAmount`, `recordedBy`,`changedBy`, `changeDate`) 
    VALUES ( '$db_mid', '$db_yid', '$db_date', '$db_month','$db_year',  '$db_descr', '$db_type','$db_income','$db_expenditure','$db_recordedby', '$postedby', '$lastupdate')");
           ...
           /* echo $fm_date."<br>";
            ...
            $update = mysqli_query($connect, "UPDATE `overall_accounts` SET `transactionDate`='$fm_date', `transactionMonth`='$fm_month', `transactionYear`='$fm_year', `itemDescription`='$fm_descr', 
             `transactionType`='$fm_type', `incomeAmount`='$fm_income', `expenditureAmount`='$fm_expenditure',`lastUpdate`='$lastupdate', `updatedBy`= '$postedby' WHERE `transactionId`='$fm_id'");
                   
                    
                    
                    if($update){
                        
                        header("Refresh: 2; url=editgeneralaccountsform.php?yid=$fm_yid&mid=$fm_mid&trid=$fm_id&page=$fm_page");
                        ...
                        echo "<a href='/generalAccounts/editgeneralaccountsform.php?yid=$fm_yid&mid=$fm_mid&trid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a></div>";
                    }
                    ...
                    echo "<a href='/generalAccounts/editgeneralaccountsform.php?yid=$fm_yid&mid=$fm_mid&trid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a><br>
                    $db_postedby<br></div>";
        ...
        echo "<a href='/generalAccounts/displaygeneralaccounts.php?yid=$fm_yid&mid=$fm_mid&trid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a></div>";

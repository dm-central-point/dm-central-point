...
if(!empty($fm_date) & !empty($fm_fname) & !empty($fm_chname) & !empty($fm_currmonth) & !empty($fm_monthpaid) & !empty($fm_curryear) & !empty($fm_yearpaid) & !empty($fm_amount)){
    ...
    if(mysqli_num_rows($select)>0){
        ...
        if(isset($_SESSION['userName'])){
                ...
                `monthPaidFor`= '$fm_monthpaid', `currentYear`= '$fm_curryear',`yearPaidFor`= '$fm_yearpaid',`amountPaid`= '$fm_amount',`outStanding`= '$fm_standing',`updatedBy`= '$postedby',`lastUpdated`= '$lastupdate' WHERE `id`='$fm_id'");
                    ...
                    if($update){
                        ...
                        header("Refresh: 2; url=/two_dimension1/accountsnewformats/editfeeform.php?yid=$fm_yid&mid=$fm_mid&incid=$fm_id&page=$fm_page");
                        ...
                        echo "<a href='editfeeform.php?yid=$fm_yid&mid=$fm_mid&incid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a></div>";
                    }
                    ...
                    echo "<a href='editfeeform.php?yid=$fm_yid&mid=$fm_mid&incid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a><br></div>";
        ...
        echo "<a href='editexpenseform.php?yid=$fm_yid&mid=$fm_mid&incid=$fm_id&page=$fm_page' class='backlink'>Please go back to the previous page!</a></div>";
